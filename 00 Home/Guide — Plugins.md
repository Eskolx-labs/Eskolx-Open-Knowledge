...
> Every plugin and built-in feature in an Eskolx vault: what it does, how you trigger it, and what happens if it disappears. "Add a plugin only when it solves a real recurring problem" is the rule; every plugin here earned its place.
...
Obsidian 1.9+ ships **Bases**. These are visual, queryable views of notes defined in `base` code blocks (the ```` ```base ```` blocks on Home and the shelves). Query properties like `type == "research"` and render as table/cards/list.
- **If Dataview disappears:** Bases keeps the dashboards working.

### Highlightr: the 10-color highlighter
`Ctrl+Shift+E` opens a palette of **10 theme-aware colors** (Soil, Wine, Grape, Forest, Harvest, Sky, Slate, Rose, Mint, Amber). `Ctrl+H` is the **native** toggle-highlight (`==text==`). This is a real background highlight. Your color choices stay local (gitignored).
...
Fallbacks don't reproduce every feature. They keep the underlying knowledge recoverable and portable.
...
[[Guide — Git and Sync]] covers how the whole thing syncs and what to do when it fights back.