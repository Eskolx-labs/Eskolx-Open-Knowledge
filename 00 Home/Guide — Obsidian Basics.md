...
- **Right sidebar**: backlinks, outline, and Related Notes. These panels show how the current note connects to the rest of the vault.
- **Bottom status bar**: Git status (last pull/push time).

## The two most important shortcuts

1. **`Ctrl+P` opens the Command Palette.** Every single thing Obsidian can do is a command. Type to filter, press Enter to run. If you forget a hotkey, this is your memory.
2. **`Ctrl+O` opens the Quick Switcher.** Open any note by typing part of its name. This is faster than clicking folders.
...
- `[[Note Name]]` is a **link to another note**. Click it to jump; Obsidian tracks it in Backlinks.
- `[[Note Name|alias]]` is a link that displays different text.
- `[[Note Name#Heading]]` links to a specific heading.
- `![[Image.png]]` **embeds** a file's contents inline (images, other notes, even tldraw scenes).
...
Open **Graph view** (command palette or the left ribbon) to see the whole vault as a network. Each note is a node, each link an edge. It's a map of the lab's thinking. Clusters you didn't create are usually concepts you *should* link.

## Where to go next

You now know enough to read and write notes. Next: [[Guide — Workflows]]. That's how the Eskolx loop actually works day to day.